...
if "MISSING_TRANSITIONS" in G[0]:
    ...
    for i in xrange(len(LMT)):
        for j in xrange(len(LMT[i])):
            if type(LMT[i][j]) == type([]):
                LMT[i][j] = sorted(LMT[i][j])
...
for i in xrange(len(LGS)):
    for j in xrange(len(LGS[i])):
        if type(LGS[i][j]) == type([]):
            LGS[i][j] = sorted(LGS[i][j])
print LGS
print LGN
...
if True == True:
    ...
    if W == "NEW":
        ...
        print "====== DSGT = ExcludeStates(DSG,LGS) ======"
        DSGT = []
        for X in DSG:
            if not(X[0] in LGS) and not(X[2] in LGS):
                DSGT.append(X)
        SemLearnLib.TrainingSemanticsModel([DSGT,VList],conffile,sdir,learn_tails = True)
    ...
    for i in xrange(len(LGS)):
        ...
        #for X in TC[0:20]: print X
        NTTT = 0
        for X in TC: 
            if True: #X[0][1] in ["Get_Gift","Add_MP4_To_Cart","Special_Offer"]:
                print X
                NTTT = NTTT + 1
            if NTTT > 20: break
    ...
    if LMT != None:
        ...
        for i in xrange(min(len(LMT),len(X))):
            LRT.append(X[i][0])
        """
        SACC = SemLearnLib.ReachabilityRepairAccuracyS(LRT,LMT)
        print "Accuracy:",SACC
        NRR = len(LMT)
        NUMSEL = len(LRT)
