...
for sd in xrange(778,788):
    ...
    for sem_mdl_type in ["SKCART","MLP","BNBayes","LR","SVM"]:
        for fn in FL:
            ...
            cmd_list.append("python src/python/make_faulty_machine_M3.py %s %d %d %d %s/faulty_model"%(mchfile,num_goals,rn,sd,mdldir))
            
            cmd_list.append("python src/python/b_restricted_reachability_repair_fast.py %s/faulty_model/result.mch %s/faulty_model/goal.txt NEW %s/faulty_model/answer.txt %s %s/repaired_model/"%(mdldir,mdldir,mdldir,conffile,mdldir))
