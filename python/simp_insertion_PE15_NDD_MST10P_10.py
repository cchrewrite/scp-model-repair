...
for sd in xrange(1005,1015):
    ...
    for fn in FL:
        ...
        #for sem_mdl_type in ["Random","BNBayes","LR","SVM"]:
            ...
            x = "y"
            while x != "y" and x != "n":
                x = raw_input("Simplifying %s --- Seed = %d --- Percentage of Missing States is %d/100 --- Semantic Model is %s? (y/n): "%(fn,sd,per_faults,sem_mdl_type))
            if x != "y": continue
            ...
            cmd_list.append("python src/python/b_repair_reconstitution.py %s/source.mch %s/result.mch %s %s/recon"%(rprdir,rprdir,conffile,rprdir))
