...
for sd in xrange(1001,1015):
    ...
    for fn in FL:
        ...
        if True:
            x = "y"
            while x != "y" and x != "n":
                x = raw_input("Making a test case for %s --- Seed = %d --- Percentage of Missing State Transitions is %d/100? (y/n): "%(fn,sd,per_faults))
            if x != "y": continue

            mchfile = datadir + "/material/" + fn

            resdir = datadir + "/dataset"
            mdldir = resdir + "/%s_NDD_MST%dP_SD%d"%(mch_id,per_faults,sd)
            ...
            cmd_list.append("python src/python/make_faulty_machine_NDD.py %s %d %d %s/faulty_model"%(mchfile,per_faults,sd,mdldir))
