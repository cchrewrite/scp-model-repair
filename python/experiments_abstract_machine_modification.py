...
for num_faults in [512]:#[128,256,512]:
    num_faults = 100
    ...
    for fn in FL:
        ...
        for sem_mdl_type in ["SKCART","Silas","BNBayes","LR","SVM","MLP"]:
            #sem_mdl_type = "SKCART"
            ...
            mdldir = resdir + "/mdl"
            logdir = resdir + "/log"
            logfile = logdir + "/%s_%s_%d.RESULT"%(mch_id,sem_mdl_type,num_faults)

            cmd_list = []
            cmd_list.append("mkdir %s"%(resdir))
            cmd_list.append("rm -r %s"%(mdldir))
            cmd_list.append("mkdir %s"%(mdldir))
            cmd_list.append("python src/python/make_faulty_machine_A.py %s %d 777 %s/faulty_model"%(mchfile,num_faults,mdldir))
