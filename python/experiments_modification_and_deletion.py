...
for sd in xrange(778,788):
    ...
    datadir = "TOSEM_Experiments/ModelRepairData"
    ...
    for fn in FL:
        ...
        for sem_mdl_type in ["SKCART"]: #["SKCART","Silas","BNBayes","LR","SVM","MLP"]:
            ...
            resdir = "TOSEM_Experiments/model_repair_results"
            mdldir = resdir + "/mdl"
            logdir = resdir + "/log"
            ...
            cmd_list.append("python src/python/b_repair_fast_modification_and_deletion.py %s/faulty_model/result.mch NEW %s/faulty_model/answer.txt %s %s/repaired_model/"%(mdldir,mdldir,conffile,mdldir))
